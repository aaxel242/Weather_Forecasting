...
def predict_with_model(model_path_lr, model_path_rf, features, rango):
    """
    Genera tarjetas de predicción para los próximos días con RF y Regresión Logística.
    Parámetros: model_path_lr (str), model_path_rf (str), features (DataFrame), rango (int días).
    """
    st.markdown("---")
...
def agente_meteorologico(temp_min, llueve): 
    ...
    Parámetros: temp_min (float °C), llueve (int 0/1). Retorna: str consejo.
    """
